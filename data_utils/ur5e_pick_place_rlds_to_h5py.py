...
def generate_h5(obs_replay, action_replay, cfg, total_traj_cnt, act_root_dir_path, edit_flag):
    ...
    for cam_name in cfg['cam_keys']:
        
        # # create video from images
        # # Define video parameters
        # height, width, _ = obs_replay['images'][cam_name][0].shape
        # fps = cfg.get('fps', 20)  # default to 20 FPS if not specified
        # video_path = os.path.join("/home/rsofnc000/Multi-Task-LFD-Framework/repo/TinyVLA/data_utils", f"{cam_name}.mp4")
        ...
        # for img in obs_replay['images'][cam_name]:
        #     # Ensure image is uint8
        #     frame = (img * 255).astype(np.uint8) if img.dtype != np.uint8 else img
        #     out.write(frame)

        # out.release()
        # print(f"Video saved for camera {cam_name} at {video_path}")
    ...
    with h5py.File(dataset_path + '.hdf5', 'w', rdcc_nbytes=1024 ** 2 * 2) as root:
        ...
        for cam_name in cfg['cam_keys']:
            _ = image.create_dataset(cam_name, (max_timesteps, cfg['cam_height'], cfg['cam_width'], 3), dtype='uint8', chunks=(1, cfg['cam_height'], cfg['cam_width'], 3), )
...
if __name__ == "__main__":
    print("Starting UR5e pick and place RLDS to HDF5 conversion...")
    parser = argparse.ArgumentParser()
    # External config file that overwrites default config
    parser.add_argument(
    "--cfg_path", default= '/home/rsofnc000/Multi-Task-LFD-Framework/repo/TinyVLA/data_utils/config/ur5_pick_place.json')
    parser.add_argument(
        "--save_dir",
    ...
    # create directory where save h5py files
    save_dir = os.path.join(args.save_dir, args.cfg_path.split('/')[-1].split('.')[0])
    ...
    # load dataset
    #ds = tfds.load(cfg['name'], data_dir=cfg['dataset_dir'], split="all")
    builder = tfds.builder_from_directory(cfg['dataset_dir'])
    ...
    framework = cfg['name'] #'droid'  # 'serl' #'droid'
    # act_pos_thres = 0.05
    # for serl framework
    # act_pos_thres = 0.05
    # for droid framework
    ...
    act_root_dir_name = f'{cfg["name"]}_succ_t0001_s-{smooth_window_size}-{smooth_order}'
    act_root_dir_path = os.path.join(args.save_dir, act_root_dir_name)
    ...
    for episode in tqdm(ds):
        ...
        raw_lang = ""
        cur_actions_dict = dict()
        edit_flag = 0
        for idx, step in enumerate(episode['steps']):
            # print(step['observation'].keys())
            if idx == 0:
                cur_actions_dict['action'] = []
                cur_actions_dict['gripper_position'] = []

            cur_actions.append(step['action'].numpy()[:-1])

            cur_obs_image['1'].append(step['observation'][cfg['cam_keys'][0]].numpy())
            cur_obs_image['2'].append(step['observation'][cfg['cam_keys'][1]].numpy())
            cur_obs_gripper_pos.append(step['observation']['gripper_state'].numpy())
            cur_obs_joint_state.append(step['observation']['joint_positions'].numpy())
            raw_lang = episode['language_instruction'].numpy().decode('utf-8')
            cur_obs_cartesian_position.append(step['observation']['EEF_state'].numpy())
            cur_actions_dict['action'].append(step['action'].numpy()[:-1])
            cur_actions_dict['gripper_position'].append(step['action'][-1][None].numpy())
        ...
        for in_ac_key in ["action"]:
            ...
            if in_ac_key == "action":
                prefix = "act_abs_" # NOTE: delta value, we keep the same key as droid
    ...
    #     print(f"traj_qvel shape: {traj_qpos.shape}")
        ...
        frontal_imgs = cur_obs_image['1']  # frontal
        wrist_imgs = cur_obs_image['2']  # wrist
        frontal_imgs = frontal_imgs[:traj_len]
        wrist_imgs = wrist_imgs[:traj_len]
        # print(f"frontal_imgs shape: {frontal_imgs.shape}")
        # print(f"wrist_imgs shape: {wrist_imgs.shape}")

        obs_replay = {
            'qpos': traj_qpos,
            # 'qvel': traj_qvel,
            'images': {'camera_front_image': frontal_imgs,
                    'camera_gripper_image': wrist_imgs}
        }
        cfg['lang_intrs'] = raw_lang
        print(cfg['lang_intrs'])
