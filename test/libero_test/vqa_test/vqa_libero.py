...
VISIBLE_2D_RELATIONS = {"left of", "right of", "next to"}

TYPE0_LISTING_TEMPLATES = [
    "LIST ONLY the objects that are present in the provided image. Do NOT describe positions or anything else.",
    "Name all visible objects in the image, and output only object names separated by commas.",
    "Provide a complete object list for this image. Return only object names.",
    "Which objects are present in this scene? Answer with only object names, comma-separated.",
    "Enumerate every object you can see in the image. Do not add explanations.",
    "Output the set of objects visible in the image. Only object names are allowed.",
    "List all objects currently visible in this robotic scene. No descriptions, only names.",
    "Identify all present objects in the image and respond only with their names.",
    "Return a comma-separated list of the objects visible in the image. Nothing else.",
    "Report all objects present in the image, using object names only and no extra text.",
]

MANUAL_TYPE1_SPECS = {
    "put the bowl on the stove": [
        {"target": "bowl", "references": ["wine bottle", "cream cheese"], "relation": "between", "gt": "between the wine bottle and the cream cheese"},
        {"target": "bowl", "references": ["wine bottle"], "relation": "in front of", "gt": "in front of the wine bottle"},
        {"target": "bowl", "references": ["plate"], "relation": "behind", "gt": "behind the plate"},
        {"target": "bowl", "references": ["wine bottle", "plate"], "relation": "between", "gt": "between the wine bottle and the plate"},
        {"target": "cream cheese", "references": ["stove"], "relation": "in front of", "gt": "in front of the stove"},
        {"target": "cream cheese", "references": ["plate"], "relation": "left of", "gt": "left of the plate"},
        {"target": "cream cheese", "references": ["bowl"], "relation": "left of", "gt": "left of the bowl"},
        {"target": "wine bottle", "references": ["bowl"], "relation": "behind", "gt": "behind the bowl"},
        {"target": "wine bottle", "references": ["wine rack"], "relation": "in front of", "gt": "in front of the wine rack"},
        {"target": "wine bottle", "references": ["wine rack"], "relation": "left of", "gt": "left of the wine rack"},
        {"target": "wine bottle", "references": ["bowl", "wine rack"], "relation": "between", "gt": "between the bowl and the wine rack"},
        {"target": "stove", "references": ["cream cheese"], "relation": "behind", "gt": "behind the cream cheese"},
        {"target": "plate", "references": ["bowl"], "relation": "in front of", "gt": "in front of the bowl"},
        {"target": "plate", "references": ["wooden cabinet"], "relation": "in front of", "gt": "in front of the wooden cabinet"},
        {"target": "plate", "references": ["wooden cabinet"], "relation": "left of", "gt": "left of the wooden cabinet"},
        {"target": "wooden cabinet", "references": ["plate"], "relation": "right of", "gt": "right of the plate"},
        {"target": "wooden cabinet", "references": ["wine rack"], "relation": "in front of", "gt": "in front of the wine rack"},
        {"target": "wine rack", "references": ["wooden cabinet"], "relation": "behind", "gt": "behind the wooden cabinet"},
        {"target": "wine rack", "references": ["wine bottle"], "relation": "right of", "gt": "right of the wine bottle"},
    ]
}
...
TASK_VARIATION_EXTRA_QUERIES = [
    ...
    },
    {
        ...
        "extra_questions": [
            {"target": "bowl", "kind": "between", "references": ["wine bottle", "cream cheese"]}
        ],
    },
    {
        ...
        "extra_questions": [
            {"target": "wine bottle", "kind": "single", "references": ["bowl"]}
        ],
    },
    {
        ...
        "extra_questions": [
            {"target": "bowl", "kind": "between", "references": ["plate", "cream cheese"]}
        ],
    },
    {
    ...
    },
    {
        ...
        "extra_questions": [
            {"target": "plate", "kind": "single", "references": ["drawer"]}
        ],
    },
    {
        ...
        "extra_questions": [
            {"target": "cream cheese", "kind": "single", "references": ["stove"]}
        ],
    },
    {
        ...
        "extra_questions": [
            {"target": "stove", "kind": "single", "references": ["cream cheese"]}
        ],
    },
    {
...
def pretty_object_name(name):
    ...
    clean = canonicalize_object_name(base).strip()
...
def find_scene_object(scene_names, query_name):
    ...
    for scene_name in scene_names:
        exact_name = canonicalize_object_name(scene_name)
        pretty_name = pretty_object_name(scene_name)
        if exact_name == q or pretty_name == q:
            exact_matches.append(scene_name)
            continue
        p_tokens = set(pretty_name.split())
        if q_tokens and q_tokens.issubset(p_tokens):
            token_matches.append(scene_name)
    if exact_matches:
        return sorted(exact_matches, key=lambda n: len(pretty_object_name(n)))[0]
    if token_matches:
        return sorted(token_matches, key=lambda n: len(pretty_object_name(n)))[0]
    return None
...
def make_type0_question(
    ...
    """
    type0_object_listing — chiede al modello di elencare tutti gli oggetti visibili.
    GT: recall-based su expected_objects.
    Metrica: quanti oggetti della lista il modello cita spontaneamente.
    """
    prompt = prompt_template
    # Filtra dalla lista completa gli oggetti "fisici" rilevanti
    # (esclude già i nomi interni MuJoCo grazie a SKIP_KEYWORDS nel chiamante)
    expected = [obj.replace("_1", "").replace("_", " ").replace("main", "").strip()
                for obj in objects_in_scene]
    return {
        "question_id": q_id,
        "type": "type0_object_listing",
        "frame": "camera",          # è sempre una domanda visiva
        "filled_prompt": prompt,
        "ground_truth": {
            "type0_object_listing": {
                "expected_objects": expected,
                "expected_object_variants": {
                    "stove": ["flat stove", "flat_stove", "flat stove 1"],
                    "flat stove": ["stove", "flat_stove", "flat stove 1"],
                },
...
def make_type1_question(
    ...
    prompt = (
        ...
        f"Use ONLY these position attributes: left side of, right side of, between, behind, in front of, next to."
    )
    ref_answer = reference_answer or f"The {obj_a} is {relation} the {obj_b}"
    equivalents = equivalent_answers or [
        f"{relation} the {obj_b}",
        f"{relation} {obj_b}",
        f"the {obj_a} is {relation} the {obj_b}",
    ]
    return {
        "question_id": q_id,
        ...
        "ground_truth": {
            "type1_spatial_relation": {
                ...
                "equivalent_answers": equivalents,
...
def make_type1_dual_ref_question(
    q_id,
    obj_a,
    ref_b,
    ref_c,
    ...
    default_answer = f"The {obj_a} is {relation} the {ref_b} and the {ref_c}"
    ref_answer = reference_answer or default_answer
    equivalents = equivalent_answers or [
        f"{relation} the {ref_b} and the {ref_c}",
        f"{relation} {ref_b} and {ref_c}",
        f"between the {ref_b} and the {ref_c}" if relation == "between" else default_answer,
    ]
    return {
        "question_id": q_id,
    ...
    return {
        ...
        "ground_truth": {
            "type2_object_identification": {
                "valid_answers": valid_answers or [target_obj],
...
def _as_spatial_description(spec: dict) -> str:
    refs = spec["references"]
    if len(refs) == 2:
        return f"between the {refs[0]} and the {refs[1]}"
    return f"{spec['relation']} the {refs[0]}"


def add_manual_questions_for_task(task_key, pretty_objects, questions, task_idx):
    specs = MANUAL_TYPE1_SPECS.get(task_key, [])
    if not specs:
        return

    objects_str = ", ".join(pretty_objects)
    for idx, spec in enumerate(specs):
        target = spec["target"]
        refs = spec["references"]
        relation = spec["relation"]
        spatial_desc = _as_spatial_description(spec)
        eq_answers = [
            f"{target} is {spatial_desc}",
            f"the {target} is {spatial_desc}",
            spatial_desc,
        ]

        if len(refs) == 2:
            questions.append(
                make_type1_dual_ref_question(
                    q_id=f"task{task_idx}_manual_q{idx}_type1_multi",
                    obj_a=target,
                    ref_b=refs[0],
                    ref_c=refs[1],
                    relation=relation,
                    objects_list=pretty_objects,
                    frame="camera",
                    reference_answer=f"The {target} is {spatial_desc}",
                    equivalent_answers=eq_answers,
                )
            )
        else:
            questions.append(
                make_type1_question(
                    q_id=f"task{task_idx}_manual_q{idx}_type1",
                    obj_a=target,
                    obj_b=refs[0],
                    relation=relation,
                    objects_list=pretty_objects,
        ...
        questions.append(
            make_type2_question(
                q_id=f"task{task_idx}_manual_q{idx}_type2",
                target_obj=target,
...
def build_task_entry(task_name, env, obs, output_dir, task_idx,
    ...
    # ── frame ────────────────────────────────────────────────────────────
    frame_rgb = obs.get("frontview_image")
    frame_source = "frontview"
    if frame_rgb is None:
        frame_rgb = obs["agentview_image"]
        frame_source = "agentview_fallback"
    if frame_rgb.ndim == 4:
    ...
    if wrist_rgb is not None:
        if wrist_rgb.ndim == 4:
            wrist_rgb = wrist_rgb[0]
        wrist_bgr = cv2.cvtColor(wrist_rgb.astype(np.uint8), cv2.COLOR_RGB2BGR)
        wrist_frame_path = os.path.join(frame_dir, f"task{task_idx:02d}_wrist_frame.png")
        cv2.imwrite(wrist_frame_path, wrist_bgr)
    ...
    questions = []
    readable_obj_names = sorted({pretty_object_name(name) for name in obj_names})
    for idx, template in enumerate(TYPE0_LISTING_TEMPLATES, start=1):
        questions.append(make_type0_question(
            q_id=f"task{task_idx}_q_listing_v{idx:02d}",
            objects_in_scene=readable_obj_names,
            prompt_template=template,
            recall_threshold=0.5,
        ))
    print(f"     [listing] {len(readable_obj_names)} oggetti attesi: {readable_obj_names}")

    task_key = canonicalize_task_command(task_name)
    if task_key in MANUAL_TYPE1_SPECS:
        add_manual_questions_for_task(task_key, readable_obj_names, questions, task_idx)
        print(f"     [manual] aggiunte {len(MANUAL_TYPE1_SPECS[task_key])} domande type1 e type2 derivate")
    else:
        for q_idx, (dist, a, b) in enumerate(nearby_pairs[:max_pairs]):
            ...
            cam_q_enabled = rel_c in VISIBLE_2D_RELATIONS or rel_c in {"behind", "in front of"}

            base_id = f"task{task_idx}_q{q_idx}"

            if rel_w != "next to":
                # World frame: generato sempre (baseline 3D)
            ...
            if cam_q_enabled:              # ← era: if rel_c != "next to"
                questions.append(make_type1_question(
            ...
            # Debug logging aggiornato
            cam_status = rel_c if cam_q_enabled else f"⛔ {rel_c} (depth→skip)"
            agreement  = "✅" if rel_w == rel_c else f"⚠️  world={rel_w}"
            print(f"     [{q_idx}] {a.split('_1')[0]} → {b.split('_1')[0]}"
                  f"  dist={dist:.3f}m  cam={cam_status}  {agreement}")
    
    # ── domande extra per task/variation (between/behind/in front of) ───
    extra_queries = TASK_QUERY_INDEX.get(task_key, [])
    if extra_queries and task_key not in MANUAL_TYPE1_SPECS:
        ...
        for eq_idx, cfg in enumerate(extra_queries):
            ...
            if not target:
                print(f"  [WARN] task {task_idx}: target non trovato per extra query: {cfg['target']}")
                continue

            refs = [find_scene_object(obj_names, r) for r in cfg["references"]]
            if any(r is None for r in refs):
                print(f"  [WARN] task {task_idx}: riferimenti non trovati per extra query: {cfg['references']}")
                continue

            if cfg["kind"] == "between" and len(refs) == 2:
                midpoint = (positions[refs[0]] + positions[refs[1]]) / 2.0
                rel_w, _ = axis_relation_world(positions[target], midpoint)
                rel_c, _ = axis_relation_camera(positions[target], midpoint, cam_right, cam_up)

                if rel_w != "next to":
                    questions.append(make_type1_dual_ref_question(
                        f"task{task_idx}_extra{eq_idx}_world_type1_multi",
                        pretty_object_name(target),
                        pretty_object_name(refs[0]),
                        pretty_object_name(refs[1]),
                        rel_w,
                        pretty_objects,
                        "world",
                    ))
                ...
                print(
                    f"     [extra{eq_idx}] between: {pretty_object_name(target)} wrt "
                    f"{pretty_object_name(refs[0])}+{pretty_object_name(refs[1])} "
                    f"(world={rel_w}, camera={rel_c})"
                )

            elif cfg["kind"] == "single" and len(refs) == 1:
                rel_w, _ = axis_relation_world(positions[target], positions[refs[0]])
                rel_c, _ = axis_relation_camera(positions[target], positions[refs[0]], cam_right, cam_up)

                if rel_w != "next to":
                    questions.append(make_type1_question(
                        f"task{task_idx}_extra{eq_idx}_world_type1",
                        pretty_object_name(target),
                        pretty_object_name(refs[0]),
                        rel_w,
                        pretty_objects,
                ...
                if rel_c != "next to":
                    questions.append(make_type1_question(
                        f"task{task_idx}_extra{eq_idx}_cam_type1",
                        pretty_object_name(target),
                        pretty_object_name(refs[0]),
                        rel_c,
                        pretty_objects,
                ...
                print(
                    f"     [extra{eq_idx}] single-ref: {pretty_object_name(target)} wrt "
                    f"{pretty_object_name(refs[0])} (world={rel_w}, camera={rel_c})"
                )
    ...
    return {
        ...
        "first_frame_source": frame_source,
        "wrist_frame_path": wrist_frame_path,
        ...
        json.dump({
            "metadata": {
                ...
                "frames": {
                    ...
                    "camera": "frontview camera frame (fallback: agentview): u=img_right, v=cam_up(behind+), z=world_up"
                },
            },
            "tasks": tasks,
